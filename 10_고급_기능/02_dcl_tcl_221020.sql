...
# 사용자 및 비밀번호 생성하기
CREATE USER test_user@[ip 주소] IDENTIFIED BY '0000';
...
# 동일한 ip 주소를 가진 모든 test_user에게 모든 데이터베이스와 테이블에 대한 SELECT / INSERT 범위 권한만 부여하기
GRANT SELECT, INSERT ON *.* TO test_user@[ip 주소];

# 동일한 ip 주소를 가진 모든 test_user에게 database_name의 모든 테이블에 대한 SELECT / INSERT 권한만 부여하기
...
-- 2. DCL(Transaction Control Language) - COMMIT / ROLLBACK / SAVEPOINT
# 트랜잭션 시작하기
START TRANSACTION;
