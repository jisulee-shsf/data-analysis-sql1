-- 1. Daily & Weekly Revenue - 2021년 10월의 평균 daily & weekly revenue 구하기
# daily revenue 출력
...
FROM(SELECT DATE_FORMAT(purchased_at - INTERVAL 9 HOUR, '%Y-%m-%U') AS date_at
     ...
     WHERE purchased_at >= '2021-10-03'
       AND purchased_at < '2020-10-31'
	 GROUP BY 1) AS inline_view_subquery;

-- 2. 요일별 평균 Revenue - 2021년 10월의 요일별 평균 revenue 구하기
/*
# 각 요일별 결제 금액 평균이 아닌, 요일별 데이터를 먼저 구한 후 서브쿼리를 통해 평균 revenue를 추출 필요
...
-- 3. 시간대별 평균 Revenue - 2021년 10월의 시간대별 평균 revenue 구하기
SELECT hour_at
	 , AVG(revenue)
FROM (SELECT DATE_FORMAT(purchased_at - INTERVAL 9 HOUR, '%Y-%m-%d') AS date_at
...
-- 4. 요일별 & 시간대별 평균 Revenue - 2021년 10월의 요일별 & 시간대별 평균 revenue 구하기
SELECT week_at
     , hour_at
	 , AVG(revenue)
FROM (SELECT DATE_FORMAT(purchased_at - INTERVAL 9 HOUR, '%Y-%m-%d') AS date_at
